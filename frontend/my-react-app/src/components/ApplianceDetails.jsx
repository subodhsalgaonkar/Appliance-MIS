...
const ApplianceDetails = () => {
    ...
    const fetchApplianceDetails = async () => {
        try {
            ...
            // console.log(response.data)//to check later comment
            // console.log(response.data.appliance.invoice_image);//to check later comment
            if(response.data.appliance.invoice_image){
                // Assuming invoice_image is an object with data as its key
                var invoiceImageData = response.data.appliance.invoice_image.data;
                var uint8Array = new Uint8Array(invoiceImageData);
                var blob = new Blob([uint8Array], { type: 'image/jpeg' }); // Adjust the MIME type accordingly
                var imageUrl = URL.createObjectURL(blob);
                // console.log(imageUrl); //to check later comment
        ...
        <div className="min-h-screen flex flex-col">
            ...
            <main className="p-6">
                <div>
                    ...
                    <button >
                      <a href={invoiceImage} target='_blank'> view image</a>                      
