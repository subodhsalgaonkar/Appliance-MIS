...
app.get('/appliance-details', async (req, res) => {
    ...
    try {
        ...
        // console.log(responseData)//to check later comment
...
app.post('/add-appliance',async(req,res) =>{
    ...
    const {u_id, g_id, a_name, purchase_date, previous_service_date, warranty_period,invoice_image} = req.body;
    ...
    try {
        ...
        // if(invoice_image){
        //     imageData = fs.readFileSync(invoice_image);
        // }
        // console.log(imageData);
        await connection.query('INSERT INTO appliance (g_id, u_id, a_name, purchase_date, previous_service_date, warranty_period, invoice_image) VALUES (?, ?, ?, ?, ?, ?, ?)',[g_id, u_id, a_name, purchase_date, previous_service_date, warranty_period, invoice_image]);

        connection.release();
        res.status(200).json({ message: 'Appliance added successfully' });
    } catch (error) {
        console.error("An error occoured while adding appliance. Please try again", error);
    }
});
